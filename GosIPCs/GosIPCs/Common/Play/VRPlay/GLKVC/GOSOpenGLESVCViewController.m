...
//static __weak GOSOpenGLESVCViewController *aSelf;
...
- (void)setEnterFullScreen:(BOOL)fullScreen{
    _enterFullScreen = fullScreen;
    self.player.enterFullScreen = fullScreen;
}
...
- (void)saveVideoScreenShot{
    GosLog(@"保存拍照 ---%s",__PRETTY_FUNCTION__);
}
//{
...
//    NSString *screenShotFilePath =nil;
...
- (void)startRecordingWithStoragePath:(NSString*)filePath
                               result:(RecordCallbackBlock)block
{
    GosLog(@"---%s----",__PRETTY_FUNCTION__);
}
//{
...
- (void)stopRecording
{
    GosLog(@"停止解码  --- %s",__PRETTY_FUNCTION__);
}
//{
...
- (void)snapshotWithStoragePath:(NSString*)filePath
                         result:(SnapshotResultBlock)block
{
    GosLog(@"---%s---",__PRETTY_FUNCTION__);
}
//{
...
//                                                         deviceType:_devType
//                                                           position:PositionMain];
    _snapshotPath = [MediaManager pathWithDevId:self.deviceId
                                     fileName:nil
                                    mediaType:GosMediaSnapshot
                                   deviceType:_devType
                                     position:PositionMain];
...
//                                                          deviceType:_devType
//                                                            position:PositionMain];
    _recordPath = [MediaManager pathWithDevId:self.deviceId
                                     fileName:nil
                                    mediaType:GosMediaRecord
                                   deviceType:_devType
                                     position:PositionMain];
...
- (void)setupContext{
    ...
    _mScreenScale = [UIScreen mainScreen].scale;
    if( _mScreenScale > 2.1 ){
        _mScreenScale = 1080.0/414;
    }
    // 暂时注释
//    if (SYName_iPhone_X == [SYDeviceInfo syDeviceName])
//    {
//        _mScreenScale = 3.0f;
//    }
    if (self.eagContext) {
    ...
    _displayWidth  = self.view.frame.size.width* _mScreenScale;
    _displayHeight = self.view.frame.size.height*_mScreenScale;
    ...
    _player.correctMode_180 = self.correctMode_180;
    _player.correctMode = self.correctMode;
    
    _player.autoRotSig = self.autoRotSig;
    _player.clickSig = self.clickSig;
    _player.enterFullScreen = self.enterFullScreen;
...
/**
 ...
 @param lpBuf buffer流
 */
- (void)startPlayYUVData:(int)lWidth
                 lHeight:(int)lHeight
                   lpBuf:(unsigned char*)lpBuf
{
    @autoreleasepool {
        
        long imageSize = lWidth * lHeight*3/2;
        
        if (lWidth!=0 && _videoWidth != lWidth) {
            _videoWidth = lWidth;
            _videoHeight = lHeight;
        ...
        NSData *yuvData = [NSData dataWithBytes:lpBuf length: imageSize];
