...
@interface PackageValidTimeApiManager () <GosApiManagerValidator, GosApiManagerParamSource>
/// 签名
...
@end
@implementation PackageValidTimeApiManager
...
+ (NSInteger)loadDataWithDeviceId:(NSString *_Nonnull)deviceId success:(void (^ _Nullable)(GosApiBaseManager * _Nonnull apiManager))successCallback fail:(void (^ _Nullable)(GosApiBaseManager * _Nonnull apiManager))failCallback {
    PackageValidTimeApiManager *api = [[PackageValidTimeApiManager alloc] init];
...
- (id)manager:(GosApiBaseManager *)manager reformData:(NSDictionary *)data {
    id temp = [data objectForKey:kCloudServiceApiRespKeyData];
    // 如果数据正确，数组中第一项即使需要的字典
    if ([temp isKindOfClass:[NSArray class]]) {
        return [PackageValidTimeApiRespModel yy_modelWithDictionary:[temp firstObject]];
    }
