...
@interface TokenCheckApiManager () <GosApiManagerValidator, GosApiManagerParamSource>
/// 签名
...
@end
@implementation TokenCheckApiManager
...
+ (NSInteger)loadDataWithDeviceId:(NSString *_Nonnull)deviceId success:(void (^ _Nullable)(GosApiBaseManager * _Nonnull apiManager))successCallback fail:(void (^ _Nullable)(GosApiBaseManager * _Nonnull apiManager))failCallback {
    TokenCheckApiManager *api = [[TokenCheckApiManager alloc] init];
...
- (id)manager:(GosApiBaseManager *)manager reformData:(NSDictionary *)data {
    return [TokenCheckApiRespModel yy_modelWithDictionary:[data objectForKey:kCloudServiceApiRespKeyData]];
}
