//  PayPalPaymentApiManager.m
//  CloudStoreSDK
//
//  Create by daniel.hu on 2018/12/11.
//  Copyright © 2018年 daniel. All rights reserved.

#import "PayPalPaymentApiManager.h"
#import "YYModel.h"
#import "PayOrderApiRespModel.h"
#import "PayPalPaymentApiRespModel.h"
...
@interface PayPalPaymentApiManager () <GosApiManagerValidator, GosApiManagerParamSource>
/// 签名
...
@implementation PayPalPaymentApiManager
...
+ (NSInteger)loadDataWithOrderNumber:(NSString *)orderNumnber success:(void (^)(GosApiBaseManager * _Nonnull))successCallback fail:(void (^)(GosApiBaseManager * _Nonnull))failCallback {
    PayPalPaymentApiManager *api = [[PayPalPaymentApiManager alloc] init];
...
- (NSString *)methodName {
    return @"paypal/check/client_token";
}
...
- (id)manager:(GosApiBaseManager *)manager reformData:(NSDictionary *)data {
    // 直接是字符串构成paypal token
    PayPalPaymentApiRespModel *result = nil;
    if ([[data objectForKey:kCloudServiceApiRespKeyData] isKindOfClass:[NSString class]]) {
        result = [[PayPalPaymentApiRespModel alloc] init];
        result.payPalToken = [data objectForKey:kCloudServiceApiRespKeyData];
    }
    return result;
}
