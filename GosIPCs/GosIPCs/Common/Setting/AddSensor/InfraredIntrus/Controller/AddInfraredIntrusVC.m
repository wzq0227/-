...
@implementation AddInfraredIntrusVC
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"Setting_InfraredIntrusion" ofType:@"gif"];
...
#pragma mark -- config
- (void)configUI{
    self.title = DPLocalizedString(@"Setting_Infrared");
    ...
    self.tips1Lab.text = DPLocalizedString(@"AddInfraredStep1");
    self.tips2Lab.text = DPLocalizedString(@"AddInfraredStep2");
    self.tips3Lab.text = DPLocalizedString(@"AddInfraredStep3");
}
           ...
           errorCode:(int)eCode
{
    ...
    switch (addStatus) {
        case AddIot_success:{        //  准备添加  执行添加IOT操作
            ...
            model.iotSensorType = GosIot_sensorInfrared;
        ...
        case AddIot_repeat:{        //  重复添加
            ...
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(callBackState:gosIOTType:sensorId:)]) {
                [weakSelf.delegate callBackState:addStatus gosIOTType:GosIot_sensorInfrared sensorId:@""];
            }
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)addIot:(BOOL)isSuccess
    ...
    if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(callBackState:gosIOTType:sensorId:)]) {
        [weakSelf.delegate callBackState:addresult gosIOTType:GosIot_sensorInfrared sensorId:m_sensorId];
    }
    [self removeTimer];
    dispatch_async(dispatch_get_main_queue(), ^{
        [weakSelf.navigationController popViewControllerAnimated:YES];
