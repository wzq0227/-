...
@property (weak, nonatomic) IBOutlet EnlargeClickButton *registerBtn;
@property (weak, nonatomic) IBOutlet EnlargeClickButton *readProtocolBtn;
@property (weak, nonatomic) IBOutlet UILabel *formatTipsLabel;
@property (weak, nonatomic) IBOutlet UITextView *protocolTextView;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *protocolWidth;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *authCodeBtnWidth;
@property (weak, nonatomic) IBOutlet UIView *registerBtnStyleView;
...
- (void)viewWillLayoutSubviews
{
    if (NO == m_isConfigGradient)
    {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            GOS_STRONG_SELF;
            [strongSelf configRegisterBtnStyle:Button_disble];
        });
    }
}
...
- (void)dealloc
{
    ...
    GosLog(@"---------- RegisterViewController dealloc ----------");
}
...
- (void)configUI
{
    self.title                         = DPLocalizedString(@"FastRegister");
    ...
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString: DPLocalizedString(@"Register_AgreeUserAgreement")];
    ...
    self.protocolTextView.attributedText = attributedString;
    self.protocolTextView.backgroundColor = [UIColor clearColor];
    self.protocolTextView.editable = NO;
    self.protocolTextView.scrollEnabled = NO;
    self.protocolTextView.delegate = self;
    self.protocolTextView.textAlignment = NSTextAlignmentCenter;
    self.protocolTextView.linkTextAttributes = @{NSForegroundColorAttributeName: GOS_COLOR_RGB(0x55AFFC),
                                                 NSUnderlineColorAttributeName: [UIColor lightGrayColor],
                                                 NSUnderlineStyleAttributeName: @(NSUnderlinePatternSolid)};
    self.protocolTextView.textContainerInset = UIEdgeInsetsMake(5, 0, 5, 0);
    [self.registerBtn setTitle:DPLocalizedString(@"Register")
...
-(BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange{
    if ([[URL scheme] isEqualToString:@"userAgrenURL"]) {
...
#pragma mark -- 设置阅读协议图标
- (void)configReadProtocolBtnIcon:(BOOL)isRemember
{
    dispatch_async(dispatch_get_main_queue(), ^{
        
        if (NO == isRemember)
        {
            [self.readProtocolBtn setImage:GOS_IMAGE(@"icon_uncheck_round_rectangle")
                                  forState:UIControlStateNormal];
        }
        else    // 记住密码
        {
            [self.readProtocolBtn setImage:GOS_IMAGE(@"icon_check_round_rectangle")
                                  forState:UIControlStateNormal];
        }
    });
}
...
#pragma mark -- 设置注册按钮
- (void)configRegisterBtnStyle:(ButtonStatus)status
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        switch (status)
        {
            case Button_disble:
            {
                [self.registerBtnStyleView gradientStartColor:BTN_DISABLE_START_COLOR
                                                     endColor:BTN_DISABLE_END_COLOR
                                                 cornerRadius:20
                                                    direction:GosGradientLeftToRight];
                [self.registerBtn setBackgroundColor:GOS_COLOR_RGB_A(0xFFFFFF, 0.5)];
            }
                break;
                
            case Button_normal:
            {
                [self.registerBtnStyleView gradientStartColor:BTN_NORMAL_START_COLOR
                                                     endColor:BTN_NORMAL_END_COLOR
                                                 cornerRadius:20
                                                    direction:GosGradientLeftToRight];
                [self.registerBtn setBackgroundColor:GOS_COLOR_RGB_A(0xFFFFFF, 0.25)];
            }
                break;
                
            case Button_highlight:
            {
                [self.registerBtnStyleView gradientStartColor:BTN_HIGHLIGHT_START_COLOR
                                                     endColor:BTN_HIGHLIGHT_END_COLOR
                                                 cornerRadius:20
                                                    direction:GosGradientLeftToRight];
                [self.registerBtn setBackgroundColor:GOS_CLEAR_COLOR];
            }
                break;
        }
    });
...
- (void)enableRegisterBtn:(BOOL)isEnable
{
    dispatch_async(dispatch_get_main_queue(), ^{
        
        self.registerBtn.enabled = isEnable;
    });
}
...
- (void)checkCanRegister
{
    if (GOS_ACCOUNT_MIN_LEN <= self.accountStr.length
        ...
        && YES == m_isReadProtocol)
    {
        m_isEnalbeRegister = YES;
        [self configRegisterBtnStyle:Button_normal];
    }
    else
    {
        m_isEnalbeRegister = NO;
        [self configRegisterBtnStyle:Button_disble];
    }
    [self enableRegisterBtn:m_isEnalbeRegister];
}
...
#pragma mark -- 注册按钮事件
- (IBAction)readProtocolBtnAction:(id)sender
{
    m_isReadProtocol = !m_isReadProtocol;
    [self configReadProtocolBtnIcon:m_isReadProtocol];
    [self checkCanRegister];
}
...
- (IBAction)registerBtnAction:(id)sender
{
    ...
    [self configRegisterBtnStyle:Button_highlight];
    [self.configSdk registWithAccount:self.accountStr
                          accountType:m_aType
         ...
         errorCode:(int)eCode
{
    ...
    else
    {
        switch (eCode)
        {
            case CONSDK_ERR_USER_IS_EXIST:  // 用户已经注册
            {
...
- (void)registAccount:(BOOL)isSuccess
            errorCode:(int)eCode
{
    GosLog(@"注册结果：%d, %d", isSuccess, eCode);
    [self configRegisterBtnStyle:Button_normal];
    if (YES == isSuccess)
    {
        [GosHUDView showSuccessWithStatus:DPLocalizedString(@"RegisterSuccess")];
    }
    else
    {
        switch (eCode)
        {
            ...
            case CONSDK_ERR_REG_FAILED: // 注册用户失败
            {
                [SVProgressHUD showErrorWithStatus:DPLocalizedString(@"AccountHasExist")];
            }
                break;
