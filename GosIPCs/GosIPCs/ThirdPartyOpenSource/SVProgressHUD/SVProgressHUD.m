...
+ (void)showInfoWithStatus:(NSString *)status forDuration:(NSTimeInterval)duration
{
    [self showImage:[self sharedView].infoImage status:status forDuration:duration];
}
...
+ (void)showImage:(UIImage*)image status:(NSString *)status forDuration:(NSTimeInterval)duration{
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        _maximumDismissTimeInterval = 1.5;
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    ...
    if(self.imageView.image == [SVProgressHUD sharedView].errorImage){
        //取消显示
