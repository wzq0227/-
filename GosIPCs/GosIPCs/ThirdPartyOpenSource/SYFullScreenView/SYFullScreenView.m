...
- (void)dealloc
{
    ...
    GosLog(@"-------- SYFullScreenView dealloc --------");
}
...
- (void)layoutSubviews
{
    ...
    if (NO == self.isGetSuperview)
    {
        ...
        self.screenCenter    = CGPointMake(CGRectGetMidX([UIScreen mainScreen].bounds),
                                           CGRectGetMidY([UIScreen mainScreen].bounds));
        self.screenBounds    = CGRectMake(0, 0, self.screenHeight, self.screenWidth);
    }
}
...
- (void)addVideoShowingNotify
{
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(videoIsShowing:)
                                                 name:kIsVideoShowingNotify
                                               object:nil];
}

- (void)videoIsShowing:(NSNotification *)notifyData
{
    if (!notifyData)
    {
        return;
    }
    NSDictionary *notifyDict = (NSDictionary *)notifyData.object;
    if (NO == [[notifyDict allKeys] containsObject:@"IsShowing"])
    {
        return;
    }
    NSNumber *isShowValue = [notifyDict objectForKey:@"IsShowing"];
    m_isVideoShowing = [isShowValue boolValue];
}

#pragma mark -- 屏幕方向监控
- (void)devOrientDidChange
{
    if (NO == m_isVideoShowing)
    {
        return;
    }
    [self setNeedsLayout];
...
//                                           self.screenHeight,
//                                           self.screenWidth);
//                CGPoint center = CGPointMake(CGRectGetMidX([UIScreen mainScreen].bounds),
//                                             CGRectGetMidY([UIScreen mainScreen].bounds));
...
//                                             CGRectGetMidY([self getControllerOfView:self].view.bounds));
                ...
                [self enterFullscreenOnOrientation:UIDeviceOrientationLandscapeLeft
                                        viewBounds:self.screenBounds
                                        viewCenter:self.screenCenter
                                     transforAngle:M_PI_2];
            }
        }
            break;
...

{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(curDevOrientation:)])
    {
        [self.delegate curDevOrientation:devOrientation];
    }
...
- (void)exitFullscreen
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(curDevOrientation:)])
    {
        [self.delegate curDevOrientation:UIDeviceOrientationPortrait];
    }
